...
public class ETLTextOutputReaderTest {
    ...
    public void testParseETLOutput() throws Exception {
        ...
        List<TallyMessage> tallyMessage = ETLTextOutputReader.parseETLOutputFile(file);
        ...
        Assert.assertEquals(2, tallyMessage.get(1).getAllLedgers().size());
        Assert.assertEquals(VoucherType.RECEIPT, tallyMessage.get(2).getVoucherType());
        Assert.assertEquals(4, tallyMessage.get(2).getAllLedgers().size());
        ...
        Assert.assertEquals(1, tallyMessage.get(4).getAllLedgers().size());
    }
}
